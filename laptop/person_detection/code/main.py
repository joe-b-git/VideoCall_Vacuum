...
def on_image(topic_name, msg, time, detector):
    ...
    for (x, y, w, h), (top_center_x, top_center_y) in zip(people_boxes, people_positions): #get the top center data.
        ...
        distance_to_center = abs(width / 2 - top_center_x) #check from the top center position.
...
def main():
    ...
    # Set callback
    subscriber.set_callback(lambda topic, msg, time: on_image(topic, msg, time, detector))
